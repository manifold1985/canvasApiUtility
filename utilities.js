...
const urlPrefixTest = process.env.PREFIX_URL;
...
const checkOverdue = async (
    ...
    while ((currAssignments = await fetchPage(url, headers, pageAssig))) {
        ...
        for (
            ...
            const restDays =
                closing -
                dayDiff(now, new Date(assignment.all_dates[0].due_at));
            const url = path.join(
                urlPrefix,
...
const fetchPage = async function (
    ...
    if (!response.ok) {
        console.log("fetchPage", response.statusText);
        return;
    }
    response = await response.json();
...
module.exports.subscribe = async function (obj) {
    ...
    for (let service of services) {
        ...
        if (newSub[service + "-sub"] && newSub[service]) {
            ...
            if (!Array.isArray(newSub[service]))
                newSub[service] = [newSub[service]];
...
const getStudents = async function (
    ...
    const url = new URL(
        path.join(urlPrefix, `api/v1/courses/${courseId}/users`)
    );
...
const createPeerGradedAssignment = async function (
    ...
    for (let i = 0; i < students.length; i++) {
        ...
        let createdQuiz = await createQuiz(
            urlPrefix,
            headers,
            courseId,
            quizData
...
const processPeerGradedAssignments = async function (
    ...
    for (let i = 0; i < unprocessedSurveys.length; i++) {
        ...
        if (currSurvey.description.match(studentIdRegExp)) {
            ...
            const assignmentId =
                currSurvey.description.match(assignmentIdRegExp)[0];
            ...
            submissions.forEach((submission) => {
                const scoreData =
                    submission.submission_history[0].submission_data.find(
                ...
                const currFeedback =
                    submission.submission_history[0].submission_data
                        .find(
                            (e) =>
                                feedbackQuestion.id.indexOf(e.question_id) != -1
                        )
                        .text.match(/(?<=>)[\w\W]+(?=<)/)[0];
