export default function (mQL, populate = false) {
  ...
  if (mQL.matches) {
    if (!populate) {
      const form = $(
        '<form action="/subscribe" method="post" class="invisible"></form>',
      );
      const table = $(
        '<table class="table table-bordered table-striped table-hover"><tr><th>Service</th><th>Subscribe</th><th colspan = "2">Courses</th></tr></table>',
      ); //Define the service list table headers.
      form.append(table);
        ...
        .then((res) => {
          if (res) {
            ...
            for (let j = 0; j < services.length; j++) {
              ...
              row.append(
                $(`<td id="${id}-1"></td>`).text(
                  `${services[j].split("-").join(" ")}`,
                ),
              ); //Define the service title cell.
              row.append(
                $(`<td id="${id}-2"></td>`).html(
                  `<input type="checkbox" name="${id}-sub" style="display: block; margin: auto">`,
                ), // Define the subsribe checkbox cell
              );
              ...
              if (types[j] == 1) {
                ...
                //Create the cross-reference cell.
                ...
                const selectCourse = makeCourseMenu(
                  res,
                  courseDiv,
                  services[j],
                  "-courses",
                ...
                for (let q = 0; q < optionLen; q++) {
                  ...
                  labels[q].text(options[q]).attr("for", ids[q]);
                  inputs[q] = $("<input></input>");
                  inputs[q]
                    .attr("id", ids[q])
                    .attr("type", inputType)
                    .attr("name", ids[q]);
                  infoDiv.append(labels[q]).append(inputs[q]);
                }
              }
              const deliver = $(
                `<button type='submit' class='btn btn-primary' formaction='/${id}'>Deliver</button>`,
              );
    ...
    form.append(
      "<button id='subscribe_button' type='submit' class='btn btn-primary btn-block animate__animated animate__zoomIn' style='margin-top: 12pt; margin-bottom: 12pt'><i class='fas fa-paper-plane'></i> Submit</button>",
    );
    ...
    $("#service-list")
      .text(
        "The mobile device version is under construction. Please use a desktop device.",
      )
      .css("font-size", "16pt")
      .append(
        '<span class="glyphicon glyphicon-wrench" style="display: block; font-size: 150px; padding-top: 50px; text-align: center" aria-hidden="true"></span>',
      );
  }
}
